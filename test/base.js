...
test('should have the same pino logging methods', t => {
  const spy = pinoSpy()
  const logger = pino()
...
test('should record logging calls', t => {
  const spy = pinoSpy()
  ...
  for (const level of levels) {
    assert.strictEqual(spy['_' + level].input.length, 1)
    assert.strictEqual(spy['_' + level].output.length, 1)
...
test('should reset recorded calls', t => {
  ...
  for (const level of levels) {
    assert.deepStrictEqual(spy['_' + level], { input: [], output: [] })
  }
})
